...
function searchcep() {
  ...
  className="w-full h-15 md:h-20 px-4 py-2 rounded-lg border focus:outline-none focus:ring focus:border-blue-300 mt-10 text-2xl bg-gray-600 text-white"
/>
      <div className="flex">
      ...
      {addresses.length > 0 && (
        <div  className="bg-gray-600">
          <h2  className="text-slg font-semibold mb-2 bg-blue-500 text-white p-2 font-bold mb-4 text-2xl font-bold " >Resultados  </h2>
          
          <ul >
            {addresses.map((address, index) => (
              <li key={index} className="pl-4 pr-4" >
                <p   className="text-2xl  mb-2 text-white font-bold">Rua: {address.address.road}</p>
                <p  className="text-lg font-semibold mb-2">Bairro: {address.address.suburb}</p>
                <p  className="text-lg font-semibold border-b-2">CEP: {address.address.postcode}</p>
              </li>
            ))}
          </ul>
        </div>
